...
class BiophysicsEngine {
  ...
  step(dt = 0.5, I_input = 0, S_stress = 0, S_reward = 0) {
    ...
    // Excitatory synaptic drive flows inward when V < E_rev (depolarizing)
    const I_syn = this.g_syn * this.y * (this.E_rev - this.V) + I_input;
    ...
    // --- D. Central Complex 16-Column Ring Attractor Continuous Recurrent ODEs ---
    // Continuous recurrent attractor dynamics (Kim et al. 2017, Su et al. 2017)
    // 16 coupled ODEs integrated via Forward Euler with Mexican-hat lateral inhibition
    const dHeading = (S_reward * 0.08 - S_stress * 0.12);
    this.headingAngle = (this.headingAngle + dHeading) % (2 * Math.PI);
    if (this.headingAngle < 0) this.headingAngle += 2 * Math.PI;

    const tau_ring = 25.0; // Column activation time constant in ms
    for (let i = 0; i < this.numColumns; i++) {
      const colAngle = (i / this.numColumns) * 2 * Math.PI;
      // Recurrent lateral connectivity: local excitation, global inhibition
      let recurrentDrive = 0;
      for (let j = 0; j < this.numColumns; j++) {
        const otherAngle = (j / this.numColumns) * 2 * Math.PI;
        const angularDist = Math.cos(colAngle - otherAngle);
        const weight = angularDist - 0.25;
        recurrentDrive += weight * this.ringState[j];
      }
      const sensoryDrive = Math.max(0, Math.cos(this.headingAngle - colAngle));
      const targetActivation = Math.max(0.02, Math.min(1.0, 0.15 + (0.5 * recurrentDrive / this.numColumns) + 0.45 * sensoryDrive));
      const dr = ((targetActivation - this.ringState[i]) / tau_ring) * dt;
      this.ringState[i] = Math.max(0.01, Math.min(1.0, this.ringState[i] + dr));
    }
    ...
    return {
      ...
      synapticCurrent: Number(I_syn.toFixed(2)),
